YEN Core version *3.1.1* is now available from:  <https://github.com/smartinsider/cryptoyen/releases>
...
Please report bugs using the issue tracker at github: <https://github.com/smartinsider/cryptoyen/issues>

Non-Mandatory Update
==============

YEN Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zYEN created before block 1050020

The transition to v2 zYEN and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zYEN to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zYEN v1 will now be able to convert that into YEN and then zYEN v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zYEN stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by YEN stakers.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zYEN code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zYEN elements if needed. (presstab)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zYEN created before block 1050020. (presstab)
