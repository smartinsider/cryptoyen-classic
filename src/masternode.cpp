...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        CTxOut vout = CTxOut(1399.99 * COIN, obfuScationPool.collateralPubKey);
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    //     if (addr.GetPort() != 37701) return false;
    // } else if (addr.GetPort() == 37701)
    //     return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 YEN tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 YEN tx -> 1 confirmation
