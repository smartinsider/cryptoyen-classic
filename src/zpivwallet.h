...
#ifndef ZIJA_ZYENWALLET_H
#define ZIJA_ZYENWALLET_H
...
public:
    CzYENWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZYEN(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZYEN(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ZIJA_ZYENWALLET_H
