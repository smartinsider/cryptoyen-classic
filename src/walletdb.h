...
public:
    ...
    bool WriteZYENSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZYENSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZYENSeed_deprecated(uint256& seed);
    bool EraseZYENSeed();
    bool EraseZYENSeed_deprecated();

    bool WriteZYENCount(const uint32_t& nCount);
    bool ReadZYENCount(uint32_t& nCount);
