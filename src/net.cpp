...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (YEN users should be well used to small numbers with many decimal places by now :)
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. YEN Core is probably already running."), addrBind.ToString());
        else
...
void RelayInv(CInv& inv)
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes){
		...
		if (pnode->nVersion >= ActiveProtocol())
            LogPrintf("#00090 - INFO - pnode->nVersion %s || ActiveProtocol - %s\n",pnode->nVersion, ActiveProtocol());			
            pnode->PushInventory(inv);
    }
}
