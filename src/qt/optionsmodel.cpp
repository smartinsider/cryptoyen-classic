...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::THEYEN);
    ...
    if (!settings.contains("nAnonymizeCryptoYenAmount"))
        settings.setValue("nAnonymizeCryptoYenAmount", 1000);

    nAnonymizeCryptoYenAmount = settings.value("nAnonymizeCryptoYenAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCryptoYenAmount"))
        SoftSetArg("-anonymizecryptoyenamount", settings.value("nAnonymizeCryptoYenAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cryptoyen.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCryptoYenAmount:
            return QVariant(nAnonymizeCryptoYenAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCryptoYenAmount:
            ...
            emit anonymizeCryptoYenAmountChanged(nAnonymizeCryptoYenAmount);
            break;
        case CoinControlFeatures:
