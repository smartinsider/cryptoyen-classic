...
std::set<std::string> ZCryptoYenControlDialog::setSelectedMints;
std::set<CMintMeta> ZCryptoYenControlDialog::setMints;

ZCryptoYenControlDialog::ZCryptoYenControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCryptoYenControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCryptoYenControlDialog::~ZCryptoYenControlDialog()
{
    delete ui;
}

void ZCryptoYenControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCryptoYenControlDialog::updateList()
{
...
void ZCryptoYenControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCryptoYenControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCryptoYen_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCryptoYenControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCryptoYenControlDialog::GetSelectedMints()
{
...
void ZCryptoYenControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
