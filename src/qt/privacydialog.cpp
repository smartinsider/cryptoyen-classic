                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zYEN ought to be enough for anybody." - Bill Gates, 2017
    ui->zYENpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzYENSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_pushButtonMintzZIJA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zYEN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zYEN, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zYEN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zYEN in ") +
...
void PrivacyDialog::on_pushButtonSpendzZIJA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zYEN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zYEN, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zYEN
        sendzYEN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zYEN
    sendzYEN();
}

void PrivacyDialog::on_pushButtonZCryptoYenControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZCryptoYenControlDialog* zCryptoYenControl = new ZCryptoYenControlDialog(this);
    zCryptoYenControl->setModel(walletModel);
    zCryptoYenControl->exec();
}

void PrivacyDialog::setZCryptoYenControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCryptoYenSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzYEN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid CryptoYen Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zYENpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zYEN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " YEN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zYEN</b>";
    ...
    // use mints from zYEN selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCryptoYenControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCryptoYenControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zYEN require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zYEN"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zYEN transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zYEN was spent successfully update the addressbook with the label
    ...
    ZCryptoYenControlDialog::setSelectedMints.clear();
    ui->labelzCryptoYenSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zYEN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " CryptoYen, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zYEN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zYEN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zYEN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zYEN "));
    ui->labelzYENAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zYEN </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zYEN </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzYENSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzYEN->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zYEN
        ui->pushButtonMintzYEN->setEnabled(false);
        ui->pushButtonMintzYEN->setToolTip(tr("zYEN is currently disabled due to maintenance."));

        // Spend zYEN
        ui->pushButtonSpendzYEN->setEnabled(false);
        ui->pushButtonSpendzYEN->setToolTip(tr("zYEN is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zYEN
        ui->pushButtonMintzYEN->setEnabled(true);
        ui->pushButtonMintzYEN->setToolTip(tr("PrivacyDialog", "Enter an amount of YEN to convert to zYEN", 0));

        // Spend zYEN
        ui->pushButtonSpendzYEN->setEnabled(true);
        ui->pushButtonSpendzYEN->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
