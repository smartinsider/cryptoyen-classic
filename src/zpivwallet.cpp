...
CzYENWallet::CzYENWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZYENSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZYENSeed_deprecated()) {
                    LogPrintf("%s: Updated zYEN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzYENWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZYENCount(nCountLastUsed);
    else if (!walletdb.ReadZYENCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzYENWallet::Lock()
{
    seedMaster = 0;
}

void CzYENWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzYENWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZYEN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzYENWallet::LoadMintPoolFromDB()
{
...
void CzYENWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzYENWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzYENWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzYENWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZYEN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzYENWallet::SeedToZYEN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzYENWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzYENWallet::GenerateDeterministicZYEN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzYENWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzYENWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
